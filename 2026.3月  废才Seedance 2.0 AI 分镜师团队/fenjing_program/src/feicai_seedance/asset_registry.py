...
SCENE_NOISE_PATTERN = re.compile(r"(闪回|回忆|场景|空间|镜头|画面)")
...
def save_asset_registry(project_root: Path, registry: dict[str, Any]) -> None:
    _save_registry(project_root, registry)
...
def _pick_scene_asset(assets: list[dict[str, Any]], name: str) -> dict[str, Any] | None:
    ...
    for asset_type in ("scene_panel", "scene"):
        candidates = [item for item in assets if item["asset_type"] == asset_type]
        for item in candidates:
            if _normalize_scene_name(item["name"]) == normalized:
                return item
        for item in candidates:
            candidate_name = _normalize_scene_name(item["name"])
            if normalized and candidate_name and (normalized in candidate_name or candidate_name in normalized):
                return item
...
def _normalize_scene_name(name: str) -> str:
    ...
    normalized = SCENE_NOISE_PATTERN.sub("", normalized)
    return normalized
