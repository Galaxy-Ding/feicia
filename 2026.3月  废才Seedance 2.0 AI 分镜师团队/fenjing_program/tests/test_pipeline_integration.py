...
class TestPipelineIntegration(unittest.TestCase):
    ...
    def test_status_and_help(self) -> None:
        ...
        self.assertIn("approve-scene-assets <ep01>", self.pipeline.command_help())
    ...
    def test_scene_commands_can_prepare_scene_assets(self) -> None:
        self.pipeline.run_start("ep01")
        self.pipeline.run_accept("ep01", "director")
        self.pipeline.run_design("ep01")
        self.pipeline.run_accept("ep01", "design")
        ...
        result = self.pipeline.run_build_reference_map("ep01")

        self.assertIn("missing_assets=0", result)
        self.assertTrue((self.root / "outputs" / "ep01" / "scenes" / "scene-asset-pack.json").exists())
    ...
    def test_auto_review_does_not_replay_generation_history(self) -> None:
        llm = HistorySpyMockLLMClient()
        pipeline = Pipeline(self.root, llm_client=llm)

        pipeline.run_start("ep01")

        business_review_call = next(
            messages for messages in llm.message_history if "业务审核" in messages[-1]["content"]
        )
        compliance_review_call = next(
            messages for messages in llm.message_history if "合规审核" in messages[-1]["content"]
        )

        self.assertEqual(len(business_review_call), 2)
        self.assertEqual(len(compliance_review_call), 2)
