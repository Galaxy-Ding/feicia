...
```bash
python3 -m feicai_seedance.cli status
python3 -m feicai_seedance.cli extract-characters demo-book ep01
python3 -m feicai_seedance.cli build-character-sheets demo-book ep01
python3 -m feicai_seedance.cli export-character-image-tasks demo-book ep01
python3 -m feicai_seedance.cli generate-character-images demo-book ep01 --browser mock
python3 -m feicai_seedance.cli review-character-images demo-book ep01
python3 -m feicai_seedance.cli approve-character-assets demo-book ep01
python3 -m feicai_seedance.cli export-character-reference-pack demo-book ep01
python3 -m feicai_seedance.cli start ep01
python3 -m feicai_seedance.cli review ep01 director
python3 -m feicai_seedance.cli accept ep01 director
python3 -m feicai_seedance.cli design ep01
python3 -m feicai_seedance.cli review ep01 design
python3 -m feicai_seedance.cli accept ep01 design
python3 -m feicai_seedance.cli export-scene-image-tasks ep01
python3 -m feicai_seedance.cli generate-scene-images ep01 --browser mock
python3 -m feicai_seedance.cli review-scene-images ep01
python3 -m feicai_seedance.cli approve-scene-assets ep01
python3 -m feicai_seedance.cli build-reference-map ep01
python3 -m feicai_seedance.cli prompt ep01
python3 -m feicai_seedance.cli review ep01 prompt
python3 -m feicai_seedance.cli accept ep01 prompt
python3 -m feicai_seedance.cli acceptance-evidence ep01
```

## 资产闭环

- `extract-characters -> build-character-sheets -> export-character-image-tasks -> generate-character-images -> review-character-images -> approve-character-assets -> export-character-reference-pack` 形成独立可运行的角色锚定链
- `export-scene-image-tasks -> generate-scene-images -> review-scene-images -> approve-scene-assets` 形成对称的场景拆图链
- `generate-character-images --browser mock` 会在 `downloads/staging/characters/<ep>/` 和 `downloads/images/characters/<ep>/` 下生成本地可审查文件
- `generate-scene-images --browser mock` 会在 `downloads/staging/scenes/<ep>/` 和 `downloads/images/scenes/<ep>/` 下生成本地可审查文件
- `approve-character-assets` 会把通过图写入 `assets/library/characters/<character_id>/`，并同步更新 `assets/registry/asset-registry.json`
- `approve-scene-assets` 会把通过图写入 `assets/library/scene-panels/`，并同步更新 `assets/registry/asset-registry.json`
