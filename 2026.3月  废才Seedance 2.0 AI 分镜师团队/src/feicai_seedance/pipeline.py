...
class Pipeline:
    ...
    def run_start(self, episode: str | None = None) -> str:
        ...
        messages = build_director_generation_prompt(self.config, self.prompts, target, script_text)
    ...
    def _review_and_revise(
        ...
        for round_index in range(1, self.config.review_max_auto_fix_rounds + 1):
            ...
            artifact = self._chat(revision_model_key, self._with_session_history(session, revision_messages))
            self.sessions.append_history(session, revision_messages + [{"role": "assistant", "content": artifact}])
        save_stage_acceptance(
            self.config.paths.reports,
            episode,
    ...
    def _run_review(self, stage_name: str, skill_key: str, artifact: str, upstream_context: str, session) -> ReviewOutcome:
        response = self._chat(
            "director_review",
            self._with_session_history(session, build_review_prompt(self.prompts, stage_name, skill_key, artifact, upstream_context)),
        )
    ...
    def _run_compliance(self, stage_name: str, artifact: str, session) -> ReviewOutcome:
        response = self._chat(
            "compliance_review",
            self._with_session_history(session, build_compliance_prompt(self.prompts, stage_name, artifact)),
        )
        payload = extract_json_object(response)
